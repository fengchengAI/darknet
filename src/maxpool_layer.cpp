...
maxpool_layer make_maxpool_layer(int batch, int h, int w, int c, int size, int stride_x, int stride_y, int padding, int maxpool_depth, int out_channels, bool antialiasing, bool avgpool, bool train)
{
    ...
    l.c = c;
    l.pad = padding;
    l.maxpool_depth = maxpool_depth;  //0
	...
	l.bflops = (l.size*l.size*l.c * l.out_h*l.out_w) / 1000000000.;  //??  仅仅是print
    if (avgpool) { //0
...
void forward_maxpool_layer(const maxpool_layer l, network_state state)
{
    ...
    if (!state.train && l.stride_x == l.stride_y) {//测试
                    ...
                    for (j = 0; j < w; ++j) {
                        ...
                        for (n = 0; n < l.size; ++n) {
                            for (m = 0; m < l.size; ++m) {
                                ...
                                bool valid = (cur_h >= 0 && cur_h < l.h && cur_w >= 0 && cur_w < l.w);
                                float val = (valid) ? state.input[index] : -FLT_MAX;
                                max_i = (val > max) ? index : max_i;
                                max = (val > max) ? val : max;
                            }
                        }
                        l.output[out_index] = max;
                        if (l.indexes) l.indexes[out_index] = max_i;  //记录最大索引，方便back_forward
                    }
                }
            }
        }
