...
layer make_yolo_layer(int batch, int w, int h, int n, int total, int* mask, int classes, int max_boxes)
{
    ...
    l.n = n; // n 为mask的数量，total是所有anchor的数量
    l.total = total;
    l.batch = batch;
    l.h = h;
    l.w = w;
    ...
    l.biases = (float*)xcalloc(total * 2, sizeof(float));// yolo层的biases存的是anchors；
    if(mask) {l.mask = mask;}
    else{
...
{
    // (l.output, l.biases, l.mask[n], box_index, i, j, l.w, l.h, state.net.w, state.net.h, l.w*l.h);
...
{// (truth, l.output, l.biases, best_n, box_index, i, j, l.w, l.h, state.net.w, state.net.h, l.delta, (2 - truth.w*truth.h), l.w*l.h, l.iou_normalizer * class_multiplier, l.iou_loss, 1, l.max_delta);
// 计算预测边界框的误差
    //i,j对应预测的中心x,y
    // i = (truth.x * l.w);  // pre 对应中心坐标y
...
{//stride = l.w*l.h
//如果一个特征图的像素点预测了多个类别，则对边框的4个值进行平均
...
{//(l.output, l.delta, class_index, class_id, l.classes, l.w*l.h, &avg_cat, l.focal_loss, l.label_smooth_eps, l.classes_multipliers);
//
    int n;
    if (delta[index + stride*class_id]){
        float y_true = 1;
        if(label_smooth_eps) y_true = y_true *  (1 - label_smooth_eps) + 0.5*label_smooth_eps;//0
        ...
        if (classes_multipliers) delta[index + stride*class_id] *= classes_multipliers[class_id];//0
        if(avg_cat) *avg_cat += output[index + stride*class_id];//0
        return;
    }
    // Focal loss
    if (focal_loss) {// 0
...
bool compare_yolo_class(float *output, int classes, int class_index, int stride, float objectness, int class_id, float conf_thresh)
{//(l.output, l.classes, class_index, l.w*
    //判断预测的box的分类结果，哪个大于conf_thresh,注意是分类结果，因为output的输出维度为4+1+50 ，所以应该是在80中的
...
void forward_yolo_layer(const layer l, network_state state)
{
    ...
    if (!state.train) return;  //推理阶段到此结束
    float tot_iou = 0;
    float tot_giou = 0;
    float tot_diou = 0;
    float tot_ciou = 0;
    ...
    int class_count = 0;
    *(l.cost) = 0;
    // 下面四个for循环是依次取n个预测的box的 x，y, w,h,confidence,class，然后依次和所有groud true 计算IOU，取IOU最大的groud_true.
    for (b = 0; b < l.batch; ++b) {// 一个batch就是一个img
        //预测框和真实框的偏差
        for (j = 0; j < l.h; ++j) {
            for (i = 0; i < l.w; ++i) {
                for (n = 0; n < l.n; ++n)  {// n 为yolo层mask的数量
                    //获得第j*w+i个cell第n个bbox的index
                    int box_index = entry_index(l, b, n*l.w*l.h + j*l.w + i, 0);
                    int class_index = entry_index(l, b, n*l.w*l.h + j*l.w + i, 4 + 1);
                    int obj_index = entry_index(l, b, n*l.w*l.h + j*l.w + i, 4);

                    //根据output，biases计算第j*w+i个cell第n个bbox在当前特征图上的相对位置[x,y],在网络输入图片上的相对宽度,高度[w,h]
                    box pred = get_yolo_box(l.output, l.biases, l.mask[n], box_index, i, j, l.w, l.h, state.net.w, state.net.h, l.w*l.h);
                    float best_match_iou = 0;  // 同下，只是这里要求“match”，即class_id_match为true
                    int best_match_t = 0;  //代码中没有用到best_match_t和best_match_iou的信息
                    float best_iou = 0;   // groud_truth和pred最大IOU的的
                    int best_t = 0;  // groud_truth和pred最大IOU的的索引

                    for (t = 0; t < l.max_boxes; ++t) {// 计算 最大IOU及其索引（计算的是预测和gt之间）
                        ...
                        if (!truth.x) break;  // continue;   测试的时候没有truth，会退出
                    ...
                    //int obj_index = entry_index(l, b, n*l.w*l.h + j*l.w + i, 4);
                    avg_anyobj += l.output[obj_index];  //这只是用来输出的。
                    l.delta[obj_index] = l.cls_normalizer * (0 - l.output[obj_index]);
                    // 计算 confidence的偏差 cls_normalizer = 1 ，iou_normalizer = 0.07

                    // 大于IOU设置的阈值 confidence梯度设为0
                    if (best_match_iou > l.ignore_thresh) {
                        l.delta[obj_index] = 0;//置信度误差
                    }
                    else if (state.net.adversarial) {//0
                        int class_index = entry_index(l, b, n*l.w*l.h + j*l.w + i, 4 + 1);
                        int stride = l.w*l.h;
                    ...
                    if (best_iou > l.truth_thresh) {//这里不会执行，因为默认ignore_thresh为1
        ...
        for (t = 0; t < l.max_boxes; ++t) {
            // /////////////////////////////////////////////////////////////////////
            //将gt_box与所有的预设anchor比较名如果存在最大iou，则判断该anchor是否在mask中
            //如果在，对gt_box的x，y对应的特征图进行设置误差项
            box truth = float_to_box_stride(state.truth + t*(4 + 1) + b*l.truths, 1);
            if (truth.x < 0 || truth.y < 0 || truth.x > 1 || truth.y > 1 || truth.w < 0 || truth.h < 0) {//报错
                char buff[256];
            ...
            int class_id = state.truth[t*(4 + 1) + b*l.truths + 4];  //    l.truths = l.max_boxes*(4 + 1);
            ...
            float best_iou = 0;
            int best_n = 0;
            int i = (truth.x * l.w);  // pre 对应中心坐标y
            int j = (truth.y * l.h);  // pred 对应中心坐标x
            box truth_shift = truth;
            truth_shift.x = truth_shift.y = 0; // 因为是预设anchor，所有只有w，h
            for (n = 0; n < l.total; ++n) { // 和所有预设anchor计算IOU
                box pred = { 0 };
                pred.w = l.biases[2 * n] / state.net.w;//biases存储的是预设anchor
            ...
            // 判断最好IOU对应的索引best_n 是否在mask里面，若没有，返回-1
            int mask_n = int_index(l.mask, best_n, l.n);//返回best_n在l.mask中出现的顺序(是否出现)
            if (mask_n >= 0) {
                int class_id = state.truth[t*(4 + 1) + b*l.truths + 4];
                if (l.map) class_id = l.map[class_id];//0

                int box_index = entry_index(l, b, mask_n*l.w*l.h + j*l.w + i, 0);
                const float class_multiplier = (l.classes_multipliers) ? l.classes_multipliers[class_id] : 1.0f;
                //l.classes_multipliers = 0 , class_multiplier = 1.0f

                //计算box四个坐标的误差项，然后返回集中ious值的集合，误差项通过l.delta指针传递
    ...
    for (b = 0; b < l.batch; ++b) {
    	...
    	//计算所有的分类loss  //因为no_iou_loss_delta已经将box信息置零了
